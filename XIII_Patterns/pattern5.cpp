...
{
    for (int i = 1; i <= n; i++) // Outer loop to iterate over the rows
    {
        for (int j = 0; j < (n - i + 1); j++) // Inner loop to iterate for columns or values in each row.
        {
            printf("* ");
        }
...
void print_pattern(int n)
{
    ...
    {
        for (int j = n; j > i; j--) // Inner loop to iterate for columns or values in each row.
        {
            printf("* ");
        }
